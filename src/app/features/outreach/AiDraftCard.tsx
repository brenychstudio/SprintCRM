...
export function AiDraftCard({ memberId, campaignId, leadId, memberStatus, latestResearch, latestMessage }: { memberId: string; campaignId: string; leadId: string; memberStatus: CampaignMemberStatus; latestResearch: ResearchSnapshot | null; latestMessage: OutboundMessage | null }) {
  ...
  const latestJobMatchesMessage = latestJob?.generation_status === 'completed' && latestMessage?.ai_generation_id === latestJob.id && latestMessage.research_snapshot_id === latestResearch?.id
  const currentCompleted = attempt?.state === 'completed'
    && attempt.researchSnapshotId === latestResearch?.id
    && (!latestMessage || latestMessage.ai_generation_id === attempt.result.job_id || attempt.result.message_version > latestMessage.version)
    ? attempt.result
    : null
  const status = attempt?.state === 'pending' || mutation.isPending ? 'generating' : attempt?.state === 'failed' ? 'failed' : researchChanged ? 'researchChanged' : (currentCompleted || latestJobMatchesMessage) ? 'completed' : !latestResearch ? 'researchRequired' : !confirmedResearchId ? 'confirmationRequired' : 'ready'
  const metadata = currentCompleted ?? (latestJobMatchesMessage ? {
  ...
  return <section data-testid="ai-draft" className="min-w-0 rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
    ...
    {metadata ? <dl className="mt-4 grid min-w-0 gap-2 text-sm sm:grid-cols-2"><div><dt className="text-zinc-500 dark:text-zinc-400">{t('campaigns.aiDraft.messageVersion')}</dt><dd data-testid="ai-draft-message-version" className="font-medium">{metadata.message_version || '—'}</dd></div><div><dt className="text-zinc-500 dark:text-zinc-400">{t('campaigns.aiDraft.researchVersion')}</dt><dd className="font-medium">{metadata.research_version || latestResearch?.version || '—'}</dd></div><div><dt className="text-zinc-500 dark:text-zinc-400">{t('campaigns.aiDraft.model')}</dt><dd className="break-words font-medium">{metadata.model}</dd></div><div><dt className="text-zinc-500 dark:text-zinc-400">{t('campaigns.aiDraft.tokens')}</dt><dd data-testid="ai-draft-total-tokens" className="font-medium">{metadata.usage.total_tokens}</dd></div><div><dt className="text-zinc-500 dark:text-zinc-400">{t('campaigns.aiDraft.duration')}</dt><dd data-testid="ai-draft-duration" className="font-medium">{metadata.duration_ms} ms</dd></div><div><dt className="text-zinc-500 dark:text-zinc-400">{t('campaigns.aiDraft.cost')}</dt><dd className="font-medium">{t('campaigns.aiDraft.costNotConfigured')}</dd></div><div className="min-w-0 sm:col-span-2"><dt className="text-zinc-500 dark:text-zinc-400">{t('campaigns.aiDraft.identifier')}</dt><dd data-testid="ai-draft-metadata-id" className="truncate font-mono text-xs" title={metadata.request_id}>{metadata.request_id.slice(0, 8)}</dd></div></dl> : null}
