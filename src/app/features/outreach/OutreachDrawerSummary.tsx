...
export function OutreachDrawerSummary({ leadId }: { leadId: string }) {
  ...
  return (
    <section className="rounded-3xl border border-zinc-200 bg-white p-5" data-testid="drawer-outreach-summary">
      ...
      <dl className="mt-4 grid gap-2 text-sm"><div className="flex justify-between gap-3"><dt className="text-zinc-500">{t('drawer.outreach.campaign')}</dt><dd className="truncate font-medium text-zinc-800">{current.campaign.name}</dd></div><div className="flex justify-between gap-3"><dt className="text-zinc-500">{t('drawer.outreach.status')}</dt><dd className="font-medium text-zinc-800">{t('campaigns.memberStatus.' + current.member.status)}</dd></div></dl>
    </section>
  )
}
