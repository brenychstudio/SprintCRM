...
export function AiResearchCard({ memberId, campaignId, leadId, website, memberStatus, researchVersion = null }: { memberId: string; campaignId: string; leadId: string; website: string | null; memberStatus: CampaignMemberStatus; researchVersion?: number | null }) {
  ...
  return <section data-testid="ai-research" className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
    ...
    {status === 'completed' && job ? <dl className="mt-4 grid gap-2 text-sm sm:grid-cols-2"><div><dt className="text-zinc-500">{t('campaigns.aiResearch.version')}</dt><dd className="font-medium">{researchVersion ?? t('campaigns.aiResearch.reviewVersion')}</dd></div><div><dt className="text-zinc-500">{t('campaigns.aiResearch.model')}</dt><dd className="font-medium">{job.model_name}</dd></div><div><dt className="text-zinc-500">{t('campaigns.aiResearch.tokens')}</dt><dd className="font-medium">{job.total_tokens ?? 0}</dd></div><div><dt className="text-zinc-500">{t('campaigns.aiResearch.duration')}</dt><dd className="font-medium">{job.duration_ms ?? 0} ms</dd></div><div><dt className="text-zinc-500">{t('campaigns.aiResearch.confidence')}</dt><dd className="font-medium">{confidence ?? '—'}</dd></div><div><dt className="text-zinc-500">{t('campaigns.aiResearch.sources')}</dt><dd className="font-medium">{sourceCount ?? '—'}</dd></div><div className="sm:col-span-2"><dt className="text-zinc-500">{t('campaigns.aiResearch.identifier')}</dt><dd className="font-mono text-xs">{(job.request_id ?? job.id).slice(0, 8)}</dd></div></dl> : null}
