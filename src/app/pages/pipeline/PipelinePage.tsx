...
export function PipelinePage() {
  ...
  const filteredLeads = useMemo(() => {
    ...
    return (leadsQuery.data ?? []).filter((lead) => {
      const matchesSearch = !query || [lead.company_name, lead.email ?? '', lead.website_domain ?? lead.website ?? '']
        .join(' ')
        .toLowerCase()
        .includes(query)

      const leadNiche = lead.niche?.trim() ?? ''
      const matchesNiche = nicheFilter === '__all'
        ? true
          ...
          <section key={stage} className="rounded-2xl border border-zinc-200 bg-white p-3">
            ...
            <div className="space-y-3">
              {grouped[stage].map((lead) => {
                ...
                return (
                  <article key={lead.id} onClick={() => setSelectedLead(lead)} className="cursor-pointer rounded-xl border border-zinc-200 bg-zinc-50 p-3 transition hover:bg-zinc-100">
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      ...
                      {stageOrder.filter((target) => target !== lead.stage).slice(0, 2).map((target) => (
                        <button
                          key={target}
                          type="button"
                          onClick={(event) => {
                          ...
                          className="rounded-lg border border-zinc-200 bg-white px-2.5 py-1.5 text-xs text-zinc-700 disabled:opacity-50"
                        >
