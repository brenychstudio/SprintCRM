...
import { campaignLeadsWizardStep, campaignWizardSteps, isCampaignWizardSubmitStep, nextCampaignWizardStep, previousCampaignWizardStep, type CampaignWizardStep } from '../../../features/campaigns/workflow'
  ...
  return (
    <section data-testid="campaign-editor">
      ...
      <form className="mt-6 rounded-3xl border border-zinc-200 bg-white p-5" onSubmit={(event) => { event.preventDefault(); saveMutation.mutate() }}>
        ...
        {step === campaignLeadsWizardStep ? <div><p className="text-sm text-zinc-600">{editing ? t('campaigns.editHint') : t('campaigns.addLeadsHint')}</p><div className="mt-4 max-h-80 space-y-2 overflow-y-auto">{activeLeads.map((lead) => {
        ...
        <div className="mt-6 flex justify-between gap-3">{step > 1 ? <button type="button" onClick={() => setStep((current) => previousCampaignWizardStep(current))} className="rounded-xl border border-zinc-200 px-4 py-2.5 text-sm">{t('campaigns.previous')}</button> : <span />}{!isCampaignWizardSubmitStep(step) ? <button type="button" onClick={() => setStep((current) => nextCampaignWizardStep(current))} className="rounded-xl bg-zinc-900 px-4 py-2.5 text-sm font-medium text-white">{t('campaigns.next')}</button> : <button type="submit" data-testid="campaign-add-leads" disabled={saveMutation.isPending} className="rounded-xl bg-zinc-900 px-4 py-2.5 text-sm font-medium text-white disabled:opacity-50">{saveMutation.isPending ? t('campaigns.saving') : editing ? t('campaigns.save') : t('campaigns.create')}</button>}</div>
      </form>
    </section>
  )
}
