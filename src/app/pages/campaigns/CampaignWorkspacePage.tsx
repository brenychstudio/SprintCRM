...
import { featureFlags, isAiDraftGenerationVisible } from '../../../features/featureFlags/featureFlags'
...
import { AiDraftCard } from '../../features/outreach/AiDraftCard'
  ...
  return <section data-testid="campaign-workspace">
    ...
    <div className="mt-6 grid gap-5 xl:grid-cols-[minmax(240px,0.35fr)_minmax(0,0.65fr)]">
      ...
      <div className="space-y-5">
        ...
        {isAiDraftGenerationVisible(featureFlags) && lead ? <AiDraftCard memberId={member.id} campaignId={campaign.id} leadId={lead.id} memberStatus={member.status} latestResearch={latestResearch} latestMessage={latestMessage} /> : null}
        <form className="rounded-3xl border border-zinc-200 bg-white p-5 dark:border-zinc-700 dark:bg-zinc-900" onSubmit={(event) => { event.preventDefault(); setActionError(null); messageMutation.mutate('draft') }}><div><h2 className="font-semibold text-zinc-900 dark:text-zinc-100">{t('campaigns.message.title')}</h2><p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">{latestMessage ? t('campaigns.message.latest', { version: latestMessage.version }) : t('campaigns.message.empty')}</p></div><div className="mt-4 grid gap-3"><Field label={t('campaigns.message.subject')} value={subject} onChange={setSubject} /><Field label={t('campaigns.message.body')} value={body} onChange={setBody} multiline /></div><div className="mt-5 flex flex-wrap gap-2"><button data-testid="message-save-draft" disabled={!body.trim() || messageMutation.isPending} className="rounded-xl border border-zinc-200 px-4 py-2.5 text-sm font-medium text-zinc-700 disabled:opacity-50 dark:border-zinc-700 dark:text-zinc-200">{t('campaigns.message.saveDraft')}</button><button data-testid="message-submit-review" type="button" disabled={!body.trim() || messageMutation.isPending} onClick={() => { setActionError(null); messageMutation.mutate('needs_review') }} className="rounded-xl bg-zinc-900 px-4 py-2.5 text-sm font-medium text-white disabled:opacity-50 dark:bg-zinc-100 dark:text-zinc-900">{t('campaigns.message.submit')}</button>{latestMessage?.status === 'needs_review' ? <button data-testid="message-approve-next" type="button" disabled={approveMutation.isPending} onClick={() => { setActionError(null); approveMutation.mutate() }} className="rounded-xl bg-emerald-700 px-4 py-2.5 text-sm font-medium text-white disabled:opacity-50">{t('campaigns.message.approveNext')}</button> : null}</div></form>
