...
function LeadDrawer({
  ...
  const [nextAction, setNextAction] = useState<NextAction>(lead.next_action)
  const [nextDate, setNextDate] = useState<string>(lead.next_action_at.slice(0, 10))
  ...
  const moveMutation = useMutation({
    mutationFn: async (targetStage: LeadStage) => {
      ...
      await logActivity({
        lead_id: lead.id,
        type: 'stage_changed',
        meta: { from: fromStage, to: targetStage, next_action: next.next_action, next_action_at: nextActionAt },
      })
      ...
      <aside className="h-full w-full max-w-xl overflow-y-auto border-l border-zinc-200 bg-white p-6 shadow-xl">
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          <button
            ...
            disabled={isBusy}
            className="rounded-xl border border-zinc-200 px-3 py-2 text-sm transition hover:bg-zinc-50 disabled:opacity-60"
          >
            {t('drawer.logTouch')}
          </button>
