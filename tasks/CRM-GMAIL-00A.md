...
## CRM-GMAIL-00A-FIX-03 pre-edit record

- **Task goal:** Align the production persistence RPC with FIX-02 capability-based granted-scope validation through one additive `CREATE OR REPLACE FUNCTION` migration, without changing OAuth, Vault, mailbox, audit, or authority behavior.
- **Current-state evidence:** The second supervised OAuth request reached persistence and terminated safely as `persistence_error`; authoritative reread found zero Gmail mailbox accounts, credential references, referenced Vault secrets, communication threads, external messages, email send requests, outbound sent transitions, reply transitions, or unexpected activities. The applied foundation migration and schema snapshot still require literal `openid` and `email` in `complete_gmail_account_connection`, before mailbox insertion and Vault creation. Production `GMAIL_CONNECTION_ENABLED` is false. Local branch and remote are clean/synchronized at `f57de5ad58ca5c6b51a526d5437cf5a9cc8465e7`; PR #22 is open, draft, mergeable, and green.
- **Files to inspect:** Applied Gmail foundation migration (read-only), current schema snapshot, migration verifier, disposable Gmail PostgreSQL proof and pgTAP suite, generated database types, Gmail task/current-state documentation, PR/CI metadata, linked migration history, and production function definition.
- **File plan:** Add the next collision-free corrective migration; synchronize only the final function body in `supabase/schema.sql`; extend pgTAP with accepted alias/send-only cases plus fail-closed broader-scope cases and zero-effect assertions; update the proof assertion count and migration application; add verifier guards for the new migration contract; update focused task/current-state evidence. The applied `20260812000001` migration and TypeScript database signature stay unchanged.
- **Implementation plan:** Preserve the exact function signature/security/search path/membership/upsert/Vault/OAuth/audit/return logic; replace only the old literal-scope predicate with exact `gmail.send` presence plus an accepted-set exclusion check; validate in disposable PostgreSQL and all repository gates; commit/push one focused change and wait for green CI; require linked history and a dry-run showing exactly one migration before apply; verify the live function definition; run disabled smoke; enable connection only after every gate passes; stop before OAuth retry.
- **Risks:** Editing applied history, scope allowlist drift between Edge and DB, null-array fail-open behavior, partial persistence on rejected authority, accidental Vault/token inspection, generated type churn despite unchanged signature, unrelated migration in dry-run, or enabling connection before the corrective contract is live.
- **Acceptance criteria:** `gmail.send` is mandatory; only `openid`, `email`, `profile`, canonical `userinfo.email/profile`, and exact `gmail.send` are accepted; identity literals may be absent; every other scope is rejected atomically; the function signature and all non-scope behavior remain byte-equivalent in intent; 24 migrations and all Node/Bridge/Edge/pgTAP gates pass; CI is green; production dry-run/apply contains exactly the corrective migration; disabled smoke has zero effects; send remains disabled.
- **Tests:** pgTAP success for OIDC aliases, canonical userinfo aliases, and send-only; rejection for missing send, readonly, modify, compose, and full-mail; aggregate zero mailbox/credential/Vault/completion effects for rejects; migration verifier contract checks; `npm ci`, migration verification, typecheck, lint, unit tests, build, Product Bridge tests, Edge checks, and `git diff --check`; linked migration list/dry-run and post-apply catalog reread.
- **Manual smoke:** Only disabled start/callback/disconnect smoke after accepted production apply. No real OAuth, disconnect/reconnect, Google/Gmail call, inbox read, draft, or send. The operator receives `READY FOR SUPERVISED CONNECT RETRY 2` only after the connection flag is safely re-enabled.
- **Proposed commit:** `fix(gmail): align persistence scope contract`
- **Documentation impact:** Append the DB/Edge mismatch, additive forward-fix, production evidence, and acceptance status here; update current state minimally. No roadmap rewrite.

### FIX-03 implementation and local validation

- The applied `20260812000001` foundation migration remains unchanged. New additive migration `20260813000001_gmail_connection_scope_contract_fix.sql` replaces only `complete_gmail_account_connection` with the same signature, SECURITY DEFINER/search path, membership guard, mailbox upsert, Vault create/update, OAuth completion, audit, and return contract.
