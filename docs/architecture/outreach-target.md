...
## OUTREACH-03C supervised draft boundary

The proposed draft operation is foreground-only and must be based on the exact latest research snapshot that the human explicitly confirms. `generate_draft` carries no narrative or CRM data from the browser. The Edge Function independently rechecks organization membership, member state, and latest snapshot, derives the minimal allowlisted lead/campaign/research fields, then uses Responses `store:false`, no tools, strict `draft_v1`, and no automatic retry. Completion creates a new immutable `outbound_messages` version with `source=ai`, `status=draft`, its `ai_generation_id`, and its `research_snapshot_id`; the CRM activity is `outreach_draft_saved`. A stale snapshot creates neither a message nor a workflow change. `AI_DRAFT_GENERATION_ENABLED` and `VITE_AI_DRAFT_GENERATION_ENABLED` default false and are independent of Gmail/controlled-send flags.
