...
The first completed production research result was technically successful but quality-rejected: Hotel Marina Badalona returned on `gpt-5.4-mini` with 9,165 input, 438 output, and 9,603 total tokens in 4,064 ms, confidence 0.92, and five valid unique sources. It nevertheless produced English for Spanish campaign/lead context and contradicted non-empty Oria House Barcelona proof context with a null case and a no-proof warning. OUTREACH-03B-QUALITY-01 introduces an auditable `research_v2`/`outreach_research_v2` contract for future immutable versions only: trusted instructions are separated from untrusted CRM/site content; language, proof-context, confidence (maximum 0.85), evidence (two to three sources), URL uniqueness, and concise notes are server-validated; `max_tool_calls` is two and output is capped at 900 tokens. Version 1 and its completed ledger/snapshot remain unchanged.
