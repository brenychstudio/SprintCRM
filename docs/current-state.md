...
`20260801000007_supervised_ai_draft_generation.sql` is proposed only and has not been applied. It adds service-role-only draft lifecycle RPCs over the existing `ai_generations` ledger. The associated Edge Function `generate_draft` is gated by both `AI_RUNTIME_ENABLED` and `AI_DRAFT_GENERATION_ENABLED`, derives allowlisted context server-side, uses strict `draft_v1` Responses output without tools or retries, preserves safe usage on rejected results, and records immutable AI `outbound_messages` versions only after the exact latest research snapshot remains current. The workspace card requires a version-specific research confirmation and is hidden unless `outreach_ops_enabled`, `ai_runtime_enabled`, and `ai_draft_generation_enabled` are all enabled. No migration has been applied, Edge Function deployed, secret changed, or provider request made by this implementation.

Final acceptance requires reviewed migration apply; `AI_DRAFT_GENERATION_ENABLED=true` in Supabase secrets; local `VITE_AI_DRAFT_GENERATION_ENABLED=true`; Edge Function deployment; an authenticated Spanish Hotel Marina Badalona draft smoke; verification of immutable Message Version 1 and manual Version 2; confirmation that no automatic needs-review/approval/Gmail/send action occurred; ledger/audit/activity review; and OpenAI usage inspection.
...
Read-only inspection after `outreach-ai-runtime` Version 9 deployed at `2026-08-08T11:01:22Z` found zero new draft `ai_generations` rows, zero requested/completed/failed draft audit events, and no outbound messages. The red Failed card visible after that deployment was stale historical-state presentation, not evidence of a new provider attempt: it reflected the immutable preceding draft job created `2026-08-02T07:43:26.733741+00:00`, request ID `e1eb420a-241c-46a4-a471-0730a05e8abc`, terminal `draft_validation:body_missing_low_pressure_cta`, 994 tokens. FIX-02 has therefore not yet been production-tested after Version 9.
...
No migration, Edge Function change/deployment, secret change, provider request, or historical-row modification is part of FIX-03. A frontend restart/redeployment and one authenticated production generation remain pending; that retest must confirm the new attempt ID and safe code distinguish browser/pre-ledger rejection, provider/validation/persistence outcomes, and a saved immutable draft without automatic send or progression beyond `draft_ready`.
