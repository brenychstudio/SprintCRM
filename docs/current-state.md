# Current state — CRM-GMAIL-00A production accepted

## CRM-GMAIL-00A Gmail account and communication foundation

CRM-GMAIL-00A is production-accepted. Additive migrations `20260812000001_gmail_account_communication_foundation.sql` and `20260813000001_gmail_connection_scope_contract_fix.sql` are applied. SprintCRM owns organization-safe Gmail mailbox identity, the explicit OAuth connect/disconnect lifecycle, Supabase Vault refresh-credential custody, and the forward-compatible communication-domain foundation. `review_reply` is understood additively but is not generated by current runtime.
...
The feature-gated `/settings` card exposes safe account metadata and explicit connect/reconnect/disconnect only. One supervised Gmail mailbox is connected in production; disconnect/reconnect preserved the same Google/mailbox identity and rotated Vault custody without creating a duplicate. Connecting Gmail does not send email and does not enable automatic action. Actual Gmail send execution, provider drafts, inbox/thread synchronization, reply ingest/classification, contacts sync, automatic follow-ups, and sequences remain unimplemented. Shared Bridge remains READ 8 / STAGED_WRITE 2 / PRIVILEGED_ACTION 0.

Implementation validation uses disposable Supabase PostgreSQL 17.6 with Vault 0.3.1 and 49 pgTAP assertions. Production acceptance verified one connected mailbox, one Vault-backed credential reference, a coherent connect→disconnect→reconnect audit chain, exact `gmail.send` capability with no broader Gmail authority, and zero communication threads, external messages, send requests, sent/replied transitions, or activity deltas. No Vault plaintext was read and no email or inbox action occurred.

The permanent Deno 2.1.12 gate discovers and checks the outreach runtime plus all three Gmail OAuth entrypoints. Generated public types retain every canonical pre-Gmail RPC, including nullable AI lifecycle fields, while adding the Gmail schema; the disposable generator is pinned to Supabase CLI 2.108.0 and followed by a fail-closed normalization/verifier for postgres-meta nullability limits. Gmail Edge imports are pinned to Supabase JS 2.97.0 and JWKS key selection uses explicit object narrowing without changing `kid`/RSA/signature-use requirements. Repository validation is green; the three connection-only OAuth functions are deployed and the server/operator connection flags are enabled. Controlled send remains disabled.

The first supervised Google OAuth attempt reached the accepted send-only consent surface and returned safely, but the callback rejected Google's granted-scope representation as `required_scope_missing`; no mailbox, credential, Vault secret, communication object, send request, activity, or outbound transition was created. FIX-02 treated the signed ID token as the independent identity proof, required exact `gmail.send` as the capability proof, and rejected every additional Gmail/full-mail scope. The requested scope set and Google Cloud configuration remained unchanged; the connection stayed disabled through the source-fix, redeploy, and kill-switch gates.

The supervised retry passed Edge scope and signed-ID-token validation but failed closed as `persistence_error`. The applied database RPC still required literal `openid` and `email`, so it rejected Google's valid canonical/omitted identity scope representation before mailbox insertion or Vault creation. Additive forward-fix `20260813000001_gmail_connection_scope_contract_fix.sql` aligned persistence with the Edge boundary: exact `gmail.send` is mandatory, identity scope literals are optional, and every scope outside the six accepted identity/send names is rejected. The historical applied foundation migration remained unchanged; function signature, security boundary, mailbox/Vault/audit behavior, requested scopes, and no-send/no-inbox authority remained unchanged. The corrected contract then passed real connect, disconnect, and reconnect acceptance.

The Google OAuth application remains **External / Testing**. The current mailbox authorization is an acceptance/test-mode authorization, not a claim of permanent verified public OAuth availability. Broader outbound operation requires separate OAuth publication/verification and deliverability readiness as applicable.
