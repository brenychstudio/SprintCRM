...
5. DB-BASELINE-02 captures the known import-schema drift (`leads.source_import_id`, `idx_leads_source_import_id`, `imports.reverted_at`, and `imports.reverted_by`) as a forward migration and updates `supabase/schema.sql`. The fields already existed in production and are used by the Imports UI.
