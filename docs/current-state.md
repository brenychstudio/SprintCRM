...
CRM-GMAIL-00A is now rebased onto the canonical Shared-bootstrap and Edge-green `main`. The permanent Deno 2.1.12 gate discovers and checks the outreach runtime plus all three Gmail OAuth entrypoints. Generated public types retain every canonical pre-Gmail RPC, including nullable AI lifecycle fields, while adding the Gmail schema; the disposable generator is pinned to Supabase CLI 2.108.0 and followed by a fail-closed normalization/verifier for postgres-meta nullability limits. Gmail Edge imports are pinned to Supabase JS 2.97.0 and JWKS key selection uses explicit object narrowing without changing `kid`/RSA/signature-use requirements. Repository validation is green; the production migration and three connection-only OAuth functions are deployed, but the server connection kill switch is disabled and no mailbox is connected.

The first supervised Google OAuth attempt reached the accepted send-only consent surface and returned safely, but the callback rejected Google's granted-scope representation as `required_scope_missing`; no mailbox, credential, Vault secret, communication object, send request, activity, or outbound transition was created. FIX-02 treats the signed ID token as the independent identity proof, requires exact `gmail.send` as the capability proof, and rejects every additional Gmail/full-mail scope. The requested scope set and Google Cloud configuration remain unchanged. Production connection stays disabled until the source fix is green, redeployed, and passes another kill-switch smoke.

The supervised retry passed Edge scope and signed-ID-token validation but failed closed as `persistence_error`. The applied database RPC still required literal `openid` and `email`, so it rejected Google's valid canonical/omitted identity scope representation before mailbox insertion or Vault creation. Additive forward-fix `20260813000001_gmail_connection_scope_contract_fix.sql` aligns persistence with the Edge boundary: exact `gmail.send` is mandatory, identity scope literals are optional, and every scope outside the six accepted identity/send names is rejected. The historical applied foundation migration is unchanged; function signature, security boundary, mailbox/Vault/audit behavior, requested scopes, and no-send/no-inbox authority are unchanged. Production connection remains disabled until the corrective migration passes CI, linked dry-run, apply, definition reread, and disabled smoke.
