...
-- OUTREACH-03C-QUALITY-06: accept the V4 structured required-proof-status
-- contract for new jobs while preserving V1, V2, and V3 as valid immutable
-- history. All prompt versions retain the unchanged draft_v1 output schema.
-- Forward-fix/rollback: replace this function in a later additive migration;
-- never rewrite ai_generations or any already-applied migration.
...
begin
  ...
  if p_campaign_member_id is null or p_actor_user_id is null or p_confirmed_research_snapshot_id is null or p_request_id is null or nullif(btrim(p_model), '') is null or p_prompt_version not in ('outreach_draft_v1', 'outreach_draft_v2', 'outreach_draft_v3', 'outreach_draft_v4') then
